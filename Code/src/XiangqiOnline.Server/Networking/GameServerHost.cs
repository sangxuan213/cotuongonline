...
public sealed class GameServerHost : IAsyncDisposable, IConnectionRegistry
{
    ...
    {
        await _tcpHost.StopAsync().ConfigureAwait(false);
        var activeConnections = _connections.Values.ToArray();
        var drainTasks = activeConnections.Select(c => c.DisposeAsync().AsTask());
        try { await Task.WhenAll(drainTasks).ConfigureAwait(false); } catch { }
    }
