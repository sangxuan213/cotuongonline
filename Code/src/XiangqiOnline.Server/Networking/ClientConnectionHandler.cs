...
public sealed class ClientConnectionHandler : IAsyncDisposable
{
    ...
    public XiangqiOnline.Server.Lobby.PlayerSession? CurrentSession { get; set; }
    ...
    private bool TryAcceptRequest(string requestId, DateTimeOffset now, out string errorCode)
    {
        lock (_requestGate)
        {
            ...
            if (!_requestIds.Add(requestId) || (CurrentSession is not null && !CurrentSession.TryRecordRequestId(requestId)))
            {
